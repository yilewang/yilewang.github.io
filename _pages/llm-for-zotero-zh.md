...
<p style="text-align:center;">
  <img src="/images/llm-for-zotero/label.png" alt="LLM for Zotero 标志：大脑图标与 Zotero 盾牌结合" style="max-width:512px;">
</p>
...
- **MinerU PDF 解析**：高保真提取可更准确地保留表格、公式和图表，并支持云端 MinerU 与本地 `mineru-api` 服务。
...
6. 打开任意 PDF 开始对话，插件会在首次使用时自动通过 MinerU 解析 PDF，并缓存结果供后续对话复用。
...
**暂停 / 取消限制：** `mineru-api` 没有提供取消或 DELETE 端点。点击 Pause 后，插件会暂停队列并中止 HTTP 等待，但服务器上已经开始的解析仍会继续运行直到结束。如果需要立即中止，例如想立刻切换 backend，请自行重启 `mineru-api` 进程。
...
- 云端 MinerU 模式下，启用解析时 PDF 会发送给 MinerU 进行解析。
- 本地 MinerU 模式下，PDF 会发送到您配置的本地或远程 `mineru-api` 服务。
