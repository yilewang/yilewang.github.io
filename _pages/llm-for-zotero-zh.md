---
layout: docs
title: "llm-for-zotero：Zotero AI 研究助手"
...
<h1 id="overview">llm-for-zotero：Zotero AI 研究助手</h1>
...
如果您不想使用服务商 API 密钥，可以从 [WebChat](#webchat-配置chatgpt--deepseek-网页同步) 或 [Codex App Server](#codex-配置chatgpt-plus-订阅用户) 开始。
...
- 如果启用 MinerU 解析，建议使用个人 MinerU API 密钥。
...
| 在浏览器中使用 ChatGPT 或 DeepSeek | 通过 Sync for Zotero 扩展使用 [WebChat](#webchat-配置chatgpt--deepseek-网页同步) | 不需要 |
...
<img src="/images/llm-for-zotero/webchat.jpeg" alt="WebChat 模式连接到 chatgpt.com 的截图">
...
<div class="rtd-warning">
  <div class="rtd-admonition-title">注意</div>
  WebChat 模式需要保持浏览器标签页打开，并确保 Sync for Zotero 扩展处于活动状态。浏览器和 Zotero 应运行在同一个桌面环境中；WebChat 目前仅支持论文对话，暂不支持文库对话。
</div>
...
- MinerU 模式下，启用解析时 PDF 会发送给 MinerU 进行解析。
...
| **MinerU 解析失败** | 添加个人 MinerU API 密钥，并重新尝试**测试连接**。 |
