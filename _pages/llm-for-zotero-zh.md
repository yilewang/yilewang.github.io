---
layout: docs
title: "llm-for-zotero：面向 Zotero 文库的研究 Agent 系统"
...
<h1 id="overview">llm-for-zotero：面向 Zotero 文库的研究 Agent 系统</h1>
...
<div class="rtd-feature-grid">
  ...
  <a class="rtd-feature-card" href="#精准图像提取">
    <strong>精准图像提取</strong>
    <p>提问某张图时，Agent 会直接从原始 PDF 裁出该图——清晰、准确，并可直接嵌入笔记。</p>
  </a>
...
1. 从 [Releases 页面](https://github.com/yilewang/llm-for-zotero/releases)下载最新 `.xpi` 文件。
2. 在 Zotero 中打开 `工具` &rarr; `附加组件` &rarr; 齿轮图标 &rarr; **从文件安装附加组件**，选择该 `.xpi` 文件。
3. 重启 Zotero。
4. 打开 `首选项` &rarr; `llm-for-zotero`，选择服务商，填写基础 URL、密钥和模型，然后点击**测试连接**。
5. 在 Zotero 中打开一篇 PDF，点击右侧工具栏的 LLM Assistant 图标。

如果您不想使用服务商 API 密钥，可以从 [WebChat](#webchat-配置chatgpt-网页同步) 或 [Codex App Server](#codex-配置chatgpt-plus-订阅用户) 开始。
...
- **使用统计**：以本地仪表板汇总论文对话和文库对话活动，并提供时间筛选、活动与 Token 图表、模型明细、CSV 导出和重置功能。
  详见[使用统计](#使用统计)。
...
- **MinerU PDF 解析**：高保真提取可更准确地保留表格、公式和图表，并支持云端 MinerU、本地 `mineru-api` 服务、批量解析、缓存修复、同步包、标签和解析过滤器。
- **精准图像提取**：图像问答与图像笔记现在使用直接从原始 PDF 裁出的图像（以 MinerU 为索引），而非 MinerU 自身的图片，从而获得更清晰、更准确的图像。详见[精准图像提取](#精准图像提取)。

感谢 [@jianghao-zhang](https://github.com/jianghao-zhang) 和 [@boltma](https://github.com/boltma) 对 Codex App Server、Claude Code 和文件上传工作流的重要贡献。
...
<div class="rtd-tip">
  ...
  本插件支持 <strong>Zotero 7</strong>、<strong>Zotero 8</strong> 和 <strong>Zotero 9</strong>，请确保您运行的是兼容版本。
</div>
...
提问时，模型会生成**基于论文内容**的答案。引用标签会在页码位置经过验证前保持保守；点击已验证的引用或基于原文片段的引用，即可跳回 Zotero 中匹配的段落，方便核实答案并快速定位相关内容。
...
打开 **Zotero 设置/首选项** &rarr; **llm-for-zotero** &rarr; **Usage**，即可查看插件记录的使用活动。

<figure class="rtd-doc-figure rtd-doc-figure--wide">
  <img src="/images/llm-for-zotero/usage/usage-statistics.png" alt="llm-for-zotero 使用统计面板，显示论文对话和文库对话汇总、问题活动热图以及每日 Token 图表" width="1240" height="1144" loading="lazy">
  <figcaption>Usage 标签页按时间汇总论文对话和文库对话活动，底层统计数据仅保存在本地 Zotero 数据库中。</figcaption>
</figure>

使用 **Overview**、**Paper chat** 和 **Library chat**，可以在综合仪表板与两类对话的详细视图之间切换。
**7d**、**30d** 和 **All** 会筛选汇总、每日 Token 图表、模型明细和详细列表；活动热图则继续显示现有历史，最长为一年。

- **Overview** 显示问题总数、不同论文和对话数量、输入/输出 Token 总数、可在问题数与论文数之间切换的活动热图、每日 Token 用量，以及标明服务商或运行时和论文/文库对话占比的模型明细。
- **Paper chat** 列出提问最多的论文及其问题数和 Token 总数。
- **Library chat** 列出 Token 用量最高的对话及其问题数和 Token 总数。

该面板只报告 Token 用量，不估算费用。
对于 Usage 标签页推出前的对话，问题和论文活动会从已保存的消息中重建，而 Token 总数会明确标记为估算值，并根据已保存的提示词大小、助手回复和已保存的推理内容计算。
如果服务商没有报告 Token，面板会将相关回合标记为未报告，或显示破折号和原因说明，而不会将其当作零。
实时统计和历史重建都不包含 WebChat 回合。

使用统计仅保存在您自己的 Zotero 数据库中，此面板不会将其发送到其他地方。
**Export CSV** 会导出当前所选时间范围内的记录行；**Reset statistics** 会永久清除全部使用记录，但不会删除对话、笔记或论文。
重置后，先前从历史对话重建的记录不会再次生成。
...
4. **嵌入提取的图像裁剪**（按需）：嵌入直接从原始 PDF 裁出的精准图像，并将其复制到附件文件夹。详见[精准图像提取](#精准图像提取)。
...
前往 [github.com/yilewang/sync-for-zotero](https://github.com/yilewang/sync-for-zotero) &rarr; **Releases**，下载最新的 `extension.zip`，解压到电脑上的文件夹。
...
- 推理/思考模式由网页端控制，而不是通过插件的推理切换按钮控制。
...
<img src="/images/llm-for-zotero/minerU.png" alt="MinerU PDF 解析结果截图">
...
对于图像相关的问题，MinerU 充当定位每张图的索引，而模型实际看到的图像则是直接从原始 PDF 裁出的精准裁剪；详见[精准图像提取](#精准图像提取)。
...
本地 MinerU 服务支持由 [@renyong18](https://github.com/renyong18) 在 [PR #152](https://github.com/yilewang/llm-for-zotero/pull/152) 中贡献。

本地模式会通过 `POST /file_parse` 将 PDF 发送到自托管 `mineru-api` 服务，并把返回的 ZIP 输出保存为与云端解析相同的本地缓存格式。默认 Base URL 为 `http://127.0.0.1:8000`。

**本地模式前置条件：**

1. 安装 MinerU 并运行 `mineru-api`；安装方式请参考 [MinerU 文档](https://github.com/opendatalab/MinerU)。
2. 确保所需模型已经下载。`mineru-api` 会在首次请求时懒加载模型，因此启动服务后第一次解析，或切换 backend 后第一次解析，通常会比稳定状态更慢。

本地设置中可以选择 `Backend`：

- `pipeline`（默认）— 通用、多语言、对 CPU 更友好。
- `vlm` — 基于 VLM，对中英文文档准确率高，需要 GPU。
- `hybrid` — 较新的高准确率混合流程，支持多语言，需要本地算力。
...
- **Exclude PDFs by Filename** 支持逗号分隔的子字符串，或用 `/slashes/` 包裹的正则表达式，可用于排除翻译副本、补充材料或其他不希望自动解析的文件。
...
<img src="/images/llm-for-zotero/figure_extraction.png" alt="直接从 PDF 原始页面检测并裁剪的图像，每张以红框标出">

当 Agent 模式回答关于某张图的问题，或保存讨论该图的笔记时，它会**直接从原始 PDF 中裁剪出精准的高分辨率图像**，仅将 MinerU 缓存用作索引。
只需提问 *"图 3 展示了什么？"*，Agent 便会从 MinerU 解析中定位该图，从原始 PDF 页面裁出干净的图像，并连同图注和各子图一起进行解读。

这与[图表解读](#图表解读)不同——后者由您自己截图。
在这里，Agent 会替您查找并裁剪图像，而保存图像到笔记时嵌入的也正是这些裁剪结果。
...
| **嵌入笔记** | 保存图像笔记时，会以 `![Figure N](…)` 形式嵌入裁剪图，并自动作为 Zotero 附件导入。 |
...
<div class="rtd-tip">
  <div class="rtd-admonition-title">提示</div>
  图像处理由 <code>analyze-figures</code> 技能控制，该技能需要启用 <a href="#agent-模式beta">Agent 模式</a>。打开 <a href="#skills-技能系统">Skills</a> 门户可调整图像的定位、解读与嵌入方式。
</div>
...
如果遇到 Bug 或难以判断的问题，请在 GitHub 上 [提交 Issue](https://github.com/yilewang/llm-for-zotero/issues)。
